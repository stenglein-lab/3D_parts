...
// dim in mm
star_radius = cap_radius * 0.75;
weapon_radius = star_radius / 3;


module make_fly_death_star()
{
	union()
	{
	/*
	difference()
	{
      sphere(r=star_radius); 
	   ...
	   translate(v=spherical_to_cartesian(r,theta,phi))
      sphere(r=weapon_radius); 
	}
	*/

   r = star_radius-weapon_radius;
   theta = 20;
   phi = 0;
...
module doors()
{
   ...
   square([window_width*1.5, window_height*2]);

   // side doors
   // translate([-window_width*1.5/2,house_length/2,house_height*0.1])
   translate([house_width/2,window_width/1.5,house_height*0])
   rotate([90,0,270])
   linear_extrude(house_width*1.1)
   square([window_width*2, window_height*1.5]);
...
module flag()
{
   ...
   {
   // pole
   translate([-house_width/2 + house_width*0.85, house_length/2 + porch_length/2, 0])
   linear_extrude(height=house_height*0.8)
   circle(r=0.5);

   // flag
   translate([-house_width/2 + house_width*0.85, house_length/2 + porch_length/2 - 0.5, house_height*0.65])
   rotate([90,0,180])
...
module label()
{
   translate([0,house_length/2 - house_length*0.6,house_height])
   rotate([0,0,180])
   linear_extrude(height=0.67)
   text("Fly-te house v1", size=4, font="Helvetica", halign="center", valign="center");
   translate([0,house_length/2 - house_length*0.4,house_height])
   rotate([0,0,180])
   linear_extrude(height=0.67)
   text("StengleinLab.org", size=4, font="Helvetica", halign="center", valign="center");
   
}

module skylights()
{
   for (x_fraction =[0.1, 0.9]) 
   {
      for (y_fraction =[0.2, 0.8])
      {
         translate([house_width*x_fraction -  house_width/2, house_length*y_fraction - house_length/2 , house_height*0.9])
         linear_extrude(height=house_height*0.2)
         circle(r=3);
      }
   }
}
